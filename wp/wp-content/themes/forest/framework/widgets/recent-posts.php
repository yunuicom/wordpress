...
class forest_Recent_Posts extends WP_Widget {

	public function __construct() {
		parent::__construct(
			'forest_rp', // Base ID
			__('Forest Recent Posts with Thumbnails', 'forest'), // Name
			array( 'description' => __( 'Display your recent posts, with a Thumbnail.', 'forest' ), ) // Args
		);
	}
	...
	if (isset($instance['title'])) :
		...
		?>
		      ...
		      <?php __('Oops, there are no posts.','forest'); ?>
 	...
 	public function form( $instance ) {
		if ( isset( $instance[ 'title' ] ) ) {
			...
			$title = __( 'Latest Articles', 'forest' );
		}
		if ( isset( $instance[ 'no_of_posts' ] ) ) {
			$no_of_posts = $instance[ 'no_of_posts' ];
		}
		else {
			$no_of_posts = __( '5', 'forest' );
		}
		?>
		<p>
		<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Title:','forest' ); ?></label> 
		<input class="widefat" id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" type="text" value="<?php echo esc_attr( $title ); ?>" />
		
		<label for="<?php echo $this->get_field_id( 'no_of_posts' ); ?>"><?php _e( 'No. of Posts:', 'forest' ); ?></label>
...
add_action( 'widgets_init', 'register_forest_widget' );  
function register_forest_widget() {  
    register_widget( 'forest_Recent_Posts' );  
}  
